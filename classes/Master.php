...
Class Master extends DBConnection {
	...
	function save_supplier(){
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
				...
				if($move){
					$data .=" , bir = '{$fname}' ";
					if(isset($_SESSION['userdata']['bir']) && is_file('../'.$_SESSION['userdata']['bir']) && $_SESSION['userdata']['id'] == $id)
						unlink('../'.$_SESSION['userdata']['bir']);
				}
			}
			if(isset($_FILES['imgg']) && $_FILES['imgg']['tmp_name'] != ''){
				$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['imgg']['name'];
				$move = move_uploaded_file($_FILES['imgg']['tmp_name'],'../'. $fname);
				if($move){
					$data .=" , mayor = '{$fname}' ";
					if(isset($_SESSION['userdata']['mayor']) && is_file('../'.$_SESSION['userdata']['mayor']) && $_SESSION['userdata']['id'] == $id)
						unlink('../'.$_SESSION['userdata']['mayor']);
				}
			}
			if(isset($_FILES['imggg']) && $_FILES['imggg']['tmp_name'] != ''){
				$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['imggg']['name'];
				$move = move_uploaded_file($_FILES['imggg']['tmp_name'],'../'. $fname);
				if($move){
					$data .=" , brgy = '{$fname}' ";
					if(isset($_SESSION['userdata']['brgy']) && is_file('../'.$_SESSION['userdata']['brgy']) && $_SESSION['userdata']['id'] == $id)
						unlink('../'.$_SESSION['userdata']['brgy']);
				}
			}
			if(isset($_FILES['imgggg']) && $_FILES['imgggg']['tmp_name'] != ''){
				$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['imgggg']['name'];
				$move = move_uploaded_file($_FILES['imgggg']['tmp_name'],'../'. $fname);
				if($move){
					$data .=" , dti = '{$fname}' ";
					if(isset($_SESSION['userdata']['dti']) && is_file('../'.$_SESSION['userdata']['dti']) && $_SESSION['userdata']['id'] == $id)
						unlink('../'.$_SESSION['userdata']['dti']);
				}
			}
			if(isset($_FILES['imggggg']) && $_FILES['imggggg']['tmp_name'] != ''){
				$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['imggggg']['name'];
				$move = move_uploaded_file($_FILES['imggggg']['tmp_name'],'../'. $fname);
				if($move){
					$data .=" , business = '{$fname}' ";
					if(isset($_SESSION['userdata']['business']) && is_file('../'.$_SESSION['userdata']['business']) && $_SESSION['userdata']['id'] == $id)
						unlink('../'.$_SESSION['userdata']['business']);
				}
			}
			extract($_POST);
